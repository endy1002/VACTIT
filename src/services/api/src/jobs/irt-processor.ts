...
export async function processIRTForExam(
...
): Promise<{ success: boolean; processed: number; message?: string; dryRunResult?: any }> {
    ...
    try {
        ...
        if (options.dryRun) {
            console.log(`[IRT Processor] DRY-RUN Mode: Skipping DB updates and notifications.`);
            return {
                success: true,
                processed: resultData.students.length,
                message: 'Dry-run completed successfully',
                dryRunResult: {
                    payloadSent: payload,
                    resultReceived: resultData
                }
            };
        }
